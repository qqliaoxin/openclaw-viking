...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "",
    // === L1+: Tooling section（L0跳过，因为L0没有工具Schema） ===
    ...(isL0
      ? []
      : [
          "## Tooling",
    ...
    ...safetySection,
    // === full only: CLI Reference（L0/L1 跳过） ===
    ...(isReduced
      ? []
      : [
    ...
    ...skillsSection,
    ...memorySection,
    // === full only: Self-Update（L0/L1 跳过） ===
    hasGateway && !isMinimal && !isReduced ? "## OpenClaw Self-Update" : "",
    hasGateway && !isMinimal && !isReduced
      ? [
    ...
    hasGateway && !isMinimal && !isReduced ? "" : "",
    "",
    // === full only: Model Aliases（L0/L1 跳过） ===
  ...
  // === full only: Silent Replies（L0/L1 跳过） ===
  if (!isMinimal && !isReduced) {
    lines.push(
      "## Silent Replies",
